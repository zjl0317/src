...
class WFunc:
    """
    Multi-joint walk function for Robot305    
    """
    ...
    def apply_velocity(self,angles,velocity,phase,x):
        ...
        if v>=0:
            if phase:
                ...
                angles["l_hip_pitch"]=-d2
                angles["r_hip_pitch"]=d2
        else:
            if phase:
                angles["l_hip_pitch"]=d2
                angles["r_hip_pitch"]=-d2
...
class Walker:
    """
    Class for making Robot305 walk
    """
    def __init__(self,robot305):
        self.robot305=robot305
        self.running=False
        ...
        self._sub_cmd_vel=rospy.Subscriber(robot305.ns+"cmd_vel",Twist,self._cb_cmd_vel,queue_size=1)
    ...
    def init_walk(self):
        ...
        if self.get_dist_to_ready()>0.02:                    
            self.robot305.set_angles_slow(self.ready_pos)        
    ...
    def _do_walk(self):
        ...
        while not rospy.is_shutdown() and (self.walking or i<n or self.is_walking()):
            ...
            self.update_velocity(self.velocity,n)
            self.robot305.set_angles(angles)
            i+=1
            if i>n:
                i=0
                p=not p
    ...
    def get_dist_to_ready(self):
        angles=self.robot305.get_angles()
...
if __name__=="__main__":
    ...
    robot305=Robot305()
    rospy.loginfo("Instantiating Robot305 Walker")
    walker=Walker(robot305)
 
    rospy.loginfo("Robot305 Walker Ready")
