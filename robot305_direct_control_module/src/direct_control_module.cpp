...
void DirectControlModule::setJointCallback(const sensor_msgs::JointState::ConstPtr &msg)
{
  ...
  if(check_collision_ == true)
  {
          ...
          joint_index_it != using_joint_name_.end(); joint_index_it++)
    {
      ...
      LinkData *robot305_link = robot305_kinematics_for_target->getLinkData(joint_name);
      if(robot305_link != NULL)
        robot305_link->joint_angle_ = target_position;
    }

    robot305_kinematics_for_target->calcForwardKinematics(0);

    double diff_length = 0.0;
    bool result = getDiff(robot305_kinematics_for_target, RIGHT_END_EFFECTOR_INDEX, BASE_INDEX, diff_length);
    if(result == true && diff_length < 0.075)
      will_be_collision_ = true;

    diff_length = 0.0;
    result = getDiff(robot305_kinematics_for_target, LEFT_END_EFFECTOR_INDEX, BASE_INDEX, diff_length);
    if(result == true && diff_length < 0.075)
      will_be_collision_ = true;
  }
                                  ...
                                  std::map<std::string, double> sensors)
{
  ...
  if(check_collision_ == true)
  {
         ...
         state_it != result_.end(); state_it++)
    {
      ...
      LinkData *robot305_link = robot305_kinematics_->getLinkData(joint_name);
      if(robot305_link != NULL)
        robot305_link->joint_angle_ = goal_position;
    }

    robot305_kinematics_->calcForwardKinematics(0);
...
bool DirectControlModule::checkSelfCollision()
{
  ...
  double diff_length = 0.0;
  bool result = getDiff(robot305_kinematics_, RIGHT_END_EFFECTOR_INDEX, BASE_INDEX, diff_length);
  ...
  diff_length = 0.0;
  result = getDiff(robot305_kinematics_, RIGHT_ELBOW_INDEX, BASE_INDEX, diff_length);
  ...
  diff_length = 0.0;
  result = getDiff(robot305_kinematics_, LEFT_END_EFFECTOR_INDEX, BASE_INDEX, diff_length);
  ...
  diff_length = 0.0;
  result = getDiff(robot305_kinematics_, LEFT_ELBOW_INDEX, BASE_INDEX, diff_length);
...
bool DirectControlModule::getDiff(OP3KinematicsDynamics *kinematics, int end_index, int base_index, double &diff)
{
  ...
  Eigen::Vector3d end_position = kinematics->robot305_link_data_[end_index]->position_;
  Eigen::Vector3d base_position = kinematics->robot305_link_data_[base_index]->position_;
